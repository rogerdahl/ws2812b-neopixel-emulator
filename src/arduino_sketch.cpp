...
#endif
  pixels.begin();
  // setBrightness() is intended for use only in setup() and not for animations.
  // It's ignored by the emulator (see
  // the README).
...
void loop()
{
  ...
  // Because I build the segment arrays in memory, I use both flash and sram for
  // the segments and 5 is max for
  // what the ATtiny85 can handle with the current code. TODO: Try initializing
  // the segments as const from literals.
  ...
  // Example of simple debugging of something that only breaks when running on
  // the device.
  //#ifdef Arduino_h
  //    debugMarker();
  //#endif
...
void smoothRunners(
  u16 runSec, u16 delayMs, struct Segment* segmentPtrIn, u8 numSegments)
{
  ...
  while (millis() < startMs + runSec * 1000UL) {
    ...
    for (u8 i = 0; i < numSegments; ++i) {
      ...
      u8 numSegmentPixels =
        segmentPtr->lengthPercent * pixels.numPixels() / 100;
...
// The values of the 4 pixels are assigned in such a way that an imagined 5th
// pixel on each side is 0.
// So, for 4 pixels:
...
u32 calcTaperedSegmentPixel(u8 numSegmentPixels, u8 pos, u32 centerColor)
{
  ...
  return pixels.Color(
    rStep * segmentIndex, gStep * segmentIndex, bStep * segmentIndex);
}
