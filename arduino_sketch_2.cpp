...
void meteors(u16 runSec, u16 delayMs, u8 numMeteors, u8 minPixels, u8 maxPixels, s16 minSpeed, s16 maxSpeed)
{
    ...
    while (millis() < startMs + (runSec * 1000UL)) {
        ...
        for (u8 i = 0; i < numMeteors; ++i) {
            ...
            drawTaperedSegment(segmentArr[i].superPos, segmentArr[i].numSegmentPixels, segmentArr[i].color, maxPixels);
...
void drawTaperedSegment(u16 superPos, u8 numSegmentPixels, u32 color, u8 numEnterExitPixels)
{
    ...
    for (u8 i = 0; i <= numSegmentPixels; ++i) {
        ...
        if (drawPos < numEnterExitPixels || drawPos >= pixels.numPixels() + numEnterExitPixels) {
            continue;
        }
